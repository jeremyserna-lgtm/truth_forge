...
class WebhookSyncHandler:
    ...
    def handle_crm_webhook(self, webhook_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            crm_id_raw = contact_data.get("id") or webhook_data.get("id")

            logger.info(f"Received CRM webhook: {event_type} for contact {crm_id_raw}")

            if event_type in ("created", "updated"):
                if crm_id_raw is None:
                    return {"status": "error", "error": "No contact ID in webhook"}
                crm_id = str(crm_id_raw)
                ...
                logger.warning(f"Contact {crm_id_raw} deleted in CRM - sync handled")
    ...
    def handle_supabase_webhook(self, webhook_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if table != "contacts_master":
                return {"status": "ignored", "reason": "Not contacts_master table"}
