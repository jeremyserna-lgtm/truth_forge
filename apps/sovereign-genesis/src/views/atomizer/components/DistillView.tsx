...
const ACCEPTED_FILE_TYPES = ".txt,.md,.json,.csv,.js,.ts,.jsx,.tsx,.py,.go,.rs,.sh,.yaml,.yml,.toml,.xml,.html,.css,.sql,.zip";

// Helper: Dimensional badge colors
...
const DistillView: React.FC<DistillViewProps> = ({
  ...
  const [_includeFrontMatter, _setIncludeFrontMatter] = useState(false);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      console.log(`[DistillView] Uploading ${filesCopy.length} file(s):`,
  ...
  const handleBulkAction = async (action: 'delete' | 'context' | 'embed' | 'synthesize') => {
    ...
    if (action === 'delete') {
      ...
      try {
        ...
        const doc = await synthesizeAtomsToDocument(selected, 'Comprehensive', 'gemini-2.5-pro');
        onAddDocument(doc);
  ...
  const handleSynthesizeDocs = async () => {
    ...
    try {
      const synthDoc = await synthesizeDocuments(selectedDocs, 'Comprehensive', 'gemini-2.5-pro', _includeFrontMatter);
