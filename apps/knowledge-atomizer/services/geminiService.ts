...
import { ollamaChat, extractJson, OLLAMA_MODELS } from './ollamaService';
import { MODEL_CONFIG, STUDIO_CONFIG, TOKEN_CONFIG } from '../config';

// Approximate token counting strategy as a fallback
...
export const MODELS: ModelConfig[] = [
  ...
  // Local Ollama Models
  {
    ...
    endpoint: MODEL_CONFIG.ollamaEndpoint,
    supportsStructuredOutput: true,
...
export const generateTextArtifact = async (
  ...
  const response = await ai.models.generateContent({
    model: MODEL_CONFIG.synthesisModel,
    contents: prompt
  });
...
async function decodeAudioData(base64: string): Promise<Blob> {
  ...
  // Gemini 2.5 TTS defaults from config
  const wavBytes = writeWavHeader(bytes, STUDIO_CONFIG.audioSampleRate, STUDIO_CONFIG.audioChannels, STUDIO_CONFIG.audioBitDepth);
...
export const distillToAtoms = async (
  ...
  try {
    ...
    // Check if this is a local model (Ollama/mlx_lm.server)
    const isLocalModel = OLLAMA_MODELS.some(m => m.id === modelId);

    if (isLocalModel) {
      // Route through local model server (auto-detects Ollama native vs OpenAI-compatible)
      const chatMessages: Array<{ role: 'system' | 'user' | 'assistant'; content: string }> = [
        { role: 'system', content: 'You are a Knowledge Extraction Engine. Respond with ONLY a valid JSON array. No markdown fences, no explanation, no text outside the JSON.' },
        { role: 'user', content: prompt }
      ];

      const result = await ollamaChat(modelId, chatMessages, {
        temperature: 0.4,
        maxTokens: 4096,
        jsonMode: true,
      });
      rawText = extractJson(result.text);
    } else {
      // Gemini path — uses structured output schema
      const ai = getClient();
      const response = await ai.models.generateContent({
        model: modelId,
        contents: prompt,
...
export const generateDynamicCommands = async (
  ...
  try {
    ...
    if (isLocalModel) {
      const result = await ollamaChat(modelId, [
        { role: 'system', content: 'Respond with ONLY a valid JSON array. No markdown fences.' },
        { role: 'user', content: prompt }
...
export const chatWithContext = async (
  ...
  if (isLocalModel) {
    ...
    const result = await ollamaChat(modelId, messages, {
      temperature: 0.7,
      maxTokens: 4096,
    });
...
export const runDebateRound = async (
  ...
  try {
    ...
    if (isLocalModel) {
      const result = await ollamaChat(modelId, [
        { role: 'system', content: 'Respond with ONLY a valid JSON object. No markdown fences.' },
        { role: 'user', content: prompt }
      ], { temperature: 0.8, maxTokens: 4096, jsonMode: true });
      resultText = extractJson(result.text);
    } else {
      const ai = getClient();
      const response = await ai.models.generateContent({
        model: modelId,
