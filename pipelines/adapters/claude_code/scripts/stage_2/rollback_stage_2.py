#!/usr/bin/env python3
"""
Rollback Script for Stage 2

This script allows non-coders to safely rollback Stage 2 data.

Usage:
    python3 rollback_stage_2.py --run-id RUN_ID [--confirm]
    python3 rollback_stage_2.py --list-runs

What it does:
  - Deletes all data from stage_2 table for the specified run_id
  - Provides clear, non-technical feedback
  - Requires confirmation before deleting
  - Can list available run IDs with --list-runs
"""
...
from shared.constants import get_full_table_id, TABLE_STAGE_2, TABLE_STAGE_3
from shared_validation import validate_table_id, validate_run_id
from google.cloud.bigquery import ScalarQueryParameter

def list_available_runs() -> bool:
    """List all available run IDs in Stage 2 table.
    ...
    """
    print("="*70)
    print("AVAILABLE RUN IDs IN STAGE 2")
    print("="*70)
    print("\nThis shows all the run IDs that exist in Stage 2.")
    ...
    try:
        ...
        if not results:
            print("   ℹ️  No data found in Stage 2 table.")
            print("   What this means: Stage 2 has no runs to rollback.")
            return True
        ...
        print("   python3 rollback_stage_2.py --run-id YOUR_RUN_ID\n")
        return True
...
def rollback_stage_2(run_id: str, confirm: bool = False) -> bool:
    """Rollback Stage 2 data for a specific run_id.
    ...
    """
    print("="*70)
    print("ROLLBACK STAGE 2")
    print("="*70)
    print("\nThis script will delete Stage 2 data for a specific run.")
    ...
    # If Stage 3 has already processed data from this run_id, rolling back Stage 2
    # would create orphaned records in Stage 3 and corrupt the pipeline
    print("\nChecking for data in later stages...")
    stage_3_table = get_full_table_id(TABLE_STAGE_3)
    try:
        validated_stage_3 = validate_table_id(stage_3_table)
        dep_query = f"""
        SELECT COUNT(*) as cnt
        FROM `{validated_stage_3}`
        WHERE run_id = @run_id
        """
        ...
        if dep_result.cnt > 0:
            print(f"   ❌ CRITICAL WARNING: Found {dep_result.cnt:,} records in Stage 3 from this run!")
            print("\n   " + "="*60)
            print("   ROLLBACK BLOCKED - DOWNSTREAM DATA EXISTS")
            print("   " + "="*60)
            print(f"\n   What this means:")
            print(f"     Stage 3 has already processed {dep_result.cnt:,} records from run '{validated_run_id}'.")
            print(f"     If we delete Stage 2 data now, those Stage 3 records would become")
            print(f"     'orphaned' - their source data would be gone, corrupting the pipeline.")
            print(f"\n   What to do:")
            print(f"     1. First run the Stage 3 rollback for this run_id:")
            print(f"        python3 rollback_stage_3.py --run-id {validated_run_id}")
            print(f"     2. Then come back and run this Stage 2 rollback again.")
            print(f"\n   Why this matters:")
            print(f"     Data pipelines must be rolled back in reverse order (Stage 3, then Stage 2)")
            ...
            print("   ✅ No downstream data found in Stage 3. It is safe to proceed.")

    except Exception as e:
        # Check if the error is because the table doesn't exist
        if "Not found" in str(e) or "404" in str(e):
            print("   ✅ Stage 3 table does not exist yet. It is safe to proceed.")
        else:
            # FIX: FAIL-SAFE - Never allow bypass of data integrity check
            # The peer review identified allowing 'force' as a critical safety hazard
            # for non-technical users who cannot understand the implications of
            # orphaning downstream records. The system must always fail-safe.
            ...
            print("   " + "="*60)
            print("\n   What this means:")
            print("     The system cannot verify if Stage 3 has data from this run.")
    ...
    # Confirm deletion
    if not confirm:
        print(f"\n⚠️  WARNING: This will delete {record_count:,} records from Stage 2")
    ...
    try:
        ...
        print(f"\n✅ ROLLBACK COMPLETE")
        print("="*70)
        print(f"Stage 2 data for run_id '{validated_run_id}' has been deleted.")
        print("\nWhat this means: The data from this run has been removed.")
        print("What to do: You can now re-run Stage 2 if needed.")
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Rollback Stage 2 data for a specific run_id"
    )
    parser.add_argument(
        "--run-id",
        type=str,
    ...
    if not args.run_id:
        ...
        print("   What to do:")
        print("      1. First, run: python3 rollback_stage_2.py --list-runs")
        print("      2. Find the run ID you want to rollback")
        print("      3. Run: python3 rollback_stage_2.py --run-id YOUR_RUN_ID")
        sys.exit(1)

    success = rollback_stage_2(args.run_id, args.confirm)
    sys.exit(0 if success else 1)
