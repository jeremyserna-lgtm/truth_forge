#!/usr/bin/env python3
"""
Rollback Script for Stage 16

This script allows non-coders to safely rollback Stage 16 data.

Usage:
    python3 rollback_stage_16.py --run-id RUN_ID [--confirm]

What it does:
  - Deletes all data from stage_16 table for the specified run_id
...
from shared.constants import get_full_table_id, TABLE_STAGE_16
from shared_validation import validate_table_id, validate_run_id
from google.cloud.bigquery import ScalarQueryParameter

def list_available_runs() -> bool:
    """List all available run IDs in Stage 16 table.
    ...
    """
    print("="*70)
    print("AVAILABLE RUN IDs IN STAGE 16")
    print("="*70)
    print("\nThis shows all the run IDs that exist in Stage 16.")
    ...
    stage_table = get_full_table_id(TABLE_STAGE_16)
    ...
    try:
        ...
        if not results:
            print("   ℹ️  No data found in Stage 16 table.")
            print("   What this means: Stage 16 has no runs to rollback.")
            return True
        ...
        print("   python3 rollback_stage_16.py --run-id YOUR_RUN_ID\n")
        return True
...
def rollback_stage_16(run_id: str, confirm: bool = False) -> bool:
    """Rollback Stage 16 data for a specific run_id.
    ...
    """
    print("="*70)
    print("ROLLBACK STAGE 16")
    print("="*70)
    print("\nThis script will delete Stage 16 data for a specific run.")
    ...
    # Confirm deletion
    if not confirm:
        print(f"\n⚠️  WARNING: This will delete {record_count:,} records from Stage 16")
    ...
    try:
        ...
        print(f"\n✅ ROLLBACK COMPLETE")
        print("="*70)
        print(f"Stage 16 data for run_id '{validated_run_id}' has been deleted.")
        print("\nWhat this means: The data from this run has been removed.")
        print("What to do: You can now re-run Stage 16 if needed.")
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Rollback Stage 16 data for a specific run_id"
    )
    parser.add_argument(
        "--run-id",
        type=str,
    ...
    if not args.run_id:
        ...
        print("   What to do:")
        print("      1. First, run: python3 rollback_stage_16.py --list-runs")
        print("      2. Find the run ID you want to rollback")
        print("      3. Run: python3 rollback_stage_16.py --run-id YOUR_RUN_ID")
        sys.exit(1)

    success = rollback_stage_16(args.run_id, args.confirm)
    sys.exit(0 if success else 1)
