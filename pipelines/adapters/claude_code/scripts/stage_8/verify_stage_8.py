#!/usr/bin/env python3
"""
Verification Script for Stage 8

This script allows non-coders to verify that Stage 8 is working correctly.

Usage:
    python3 verify_stage_8.py [--run-id RUN_ID]

What it checks:
  - Table exists
  - Has records
  - L4 Sentence entities created correctly
"""
...
from shared.constants import get_full_table_id, TABLE_STAGE_8, TABLE_STAGE_7
from shared_validation import validate_table_id, validate_run_id

def verify_stage_8(run_id: str = None) -> bool:
    """Verify Stage 8 is working correctly.

    Returns:
        True if all checks pass, False otherwise
    """
    print("="*70)
    print(f"VERIFYING STAGE 8")
    print("="*70)
    print("\nThis script checks if Stage 8 worked correctly.")
    ...
    try:
        ...
        print(f"   Problem: {e}")
        print("\n   What this means: The system cannot check if Stage 8 worked.")
    ...
    try:
        ...
        if row_count == 0:
            if validated_run_id:
                ...
                print("   What to do:")
                print("      1. Check if Stage 8 ran successfully for this run_id")
                print("      2. Verify the run_id is correct")
                print("      3. Check Stage 8 logs for errors")
            else:
                print("   ⚠️  WARNING: Table exists but has no data")
                print("   What this means: Stage 8 ran but didn't create any records.")
        ...
        print(f"   Problem: {e}")
        print("   What this means: Stage 8 hasn't run yet or failed to create the table.")
        print("   What to do: Run Stage 8 first, then check for errors.")
        return False


    # Check 2: L4 Sentence entities have correct level
    print("\n2. Checking that all records are Level 4 (Sentence) entities...")
    try:
        ...
        if result.wrong_level_count > 0:
            ...
            print("   What this means: Some records are not Level 4 (Sentence) as they should be.")
            print("   What to do: This is a data corruption issue - contact support.")
            all_checks_passed = False
        else:
            print(f"   ✅ All {result.total:,} records are Level 4 (Sentence) entities")
    except Exception as e:
        print(f"   ❌ Error checking levels: {e}")
        all_checks_passed = False

    # Check 3: Parent links are valid (point to L5 messages)
    print("\n3. Checking that sentence parent links point to valid messages...")
    try:
        ...
        SELECT
            ...
            COUNTIF(parent_id NOT IN (SELECT entity_id FROM `{validated_stage_7}`)) as orphaned_sentences
        FROM `{validated_table}`
        {where_clause}
        """
        result = list(client.query(query, job_config=job_config).result())[0]
        ...
        if result.null_parents > 0:
            print(f"   ❌ Found {result.null_parents} sentences with no parent")
            print("   What this means: Some sentences don't know which message they belong to.")
            print("   What to do: This is a data quality issue - run Stage 8 again.")
            all_checks_passed = False
        elif result.orphaned_sentences > 0:
            print(f"   ❌ Found {result.orphaned_sentences} sentences pointing to non-existent messages")
            print("   What this means: Some sentences reference messages that don't exist.")
            print("   What to do: Run Stage 7 first to create messages, then run Stage 8.")
            all_checks_passed = False
        else:
            print(f"   ✅ All {result.total_sentences:,} sentences have valid parent links")
    except Exception as e:
    ...
    if all_checks_passed:
        print("✅ ALL CHECKS PASSED - Stage 8 is working correctly!")
        print("="*70)
        return True
    else:
        ...
        print("  3. Run Stage 8 again if needed")
...
def main():
    ...
    success = verify_stage_8(args.run_id)
