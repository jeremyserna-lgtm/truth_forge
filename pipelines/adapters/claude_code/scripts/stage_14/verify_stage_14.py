#!/usr/bin/env python3
"""
Verification Script for Stage 14

This script allows non-coders to verify that Stage 14 is working correctly.

Usage:
    python3 verify_stage_14.py [--run-id RUN_ID]
...
from shared.constants import get_full_table_id, TABLE_STAGE_14
from shared_validation import validate_table_id, validate_run_id

def verify_stage_14(run_id: str = None) -> bool:
    """Verify Stage 14 is working correctly.
    
    Returns:
        True if all checks pass, False otherwise
    """
    print("="*70)
    print(f"VERIFYING STAGE 14 - Entity Promotion")
    print("="*70)
    print("\nThis script checks if Stage 14 worked correctly.")
    ...
    try:
        ...
        print(f"   Problem: {e}")
        print("\n   What this means: The system cannot check if Stage 14 worked.")
    ...
    try:
        ...
        if row_count == 0:
            if validated_run_id:
                ...
                print("   What to do:")
                print("      1. Check if Stage 14 ran successfully for this run_id")
                print("      2. Verify the run_id is correct")
                print("      3. Check Stage 14 logs for errors")
            else:
                print("   ⚠️  WARNING: Table exists but has no data")
                print("   What this means: Stage 14 ran but didn't create any records.")
        ...
        print(f"   Problem: {e}")
        print("   What this means: Stage 14 hasn't run yet or failed to create the table.")
        print("   What to do: Run Stage 14 first, then check for errors.")
        return False


    # Check 2: Schema correct (check required fields exist)
    print("\n2. Checking if schema is correct...")
    try:
        table = client.get_table(validated_table)
        required_fields = ['entity_id', 'level', 'parent_id', 'run_id']
        ...
        if missing_fields:
            ...
            print("   What this means: The table schema is incomplete.")
            print("   What to do: Re-run Stage 14 to ensure schema is created correctly.")
            all_checks_passed = False
        else:
            print(f"   ✅ Schema is correct - all required fields present")
            print(f"   ✅ Table has {len(existing_fields)} fields total")
    except Exception as e:
    ...
    if all_checks_passed:
        print("✅ ALL CHECKS PASSED - Stage 14 is working correctly!")
        print("="*70)
        return True
    else:
        ...
        print("  3. Run Stage 14 again if needed")
...
def main():
    ...
    success = verify_stage_14(args.run_id)
