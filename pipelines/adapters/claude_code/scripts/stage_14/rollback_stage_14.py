#!/usr/bin/env python3
"""
Rollback Script for Stage 14

This script allows non-coders to safely rollback Stage 14 data.

Usage:
    python3 rollback_stage_14.py --run-id RUN_ID [--confirm]

What it does:
  - Deletes all data from stage_14 table for the specified run_id
...
from shared.constants import get_full_table_id, TABLE_STAGE_14, TABLE_STAGE_15
from shared_validation import validate_table_id, validate_run_id
from google.cloud.bigquery import ScalarQueryParameter

def list_available_runs() -> bool:
    """List all available run IDs in Stage 14 table.
    ...
    """
    print("="*70)
    print("AVAILABLE RUN IDs IN STAGE 14")
    print("="*70)
    print("\nThis shows all the run IDs that exist in Stage 14.")
    ...
    stage_table = get_full_table_id(TABLE_STAGE_14)
    ...
    try:
        ...
        if not results:
            print("   ℹ️  No data found in Stage 14 table.")
            print("   What this means: Stage 14 has no runs to rollback.")
            return True
        ...
        print("   python3 rollback_stage_14.py --run-id YOUR_RUN_ID\n")
        return True
...
def rollback_stage_14(run_id: str, confirm: bool = False) -> bool:
    """Rollback Stage 14 data for a specific run_id.
    ...
    """
    print("="*70)
    print("ROLLBACK STAGE 14")
    print("="*70)
    print("\nThis script will delete Stage 14 data for a specific run.")
    ...
    # If Stage 15 has already processed data from this run_id, rolling back Stage 14
    # would create orphaned records in Stage 15 and corrupt the pipeline
    print("\nChecking for data in later stages...")
    stage_15_table = get_full_table_id(TABLE_STAGE_15)
    try:
        validated_stage_15 = validate_table_id(stage_15_table)
        dep_query = f"""
        SELECT COUNT(*) as cnt
        FROM `{validated_stage_15}`
        WHERE run_id = @run_id
        """
        ...
        if dep_result.cnt > 0:
            print(f"   ❌ CRITICAL WARNING: Found {dep_result.cnt:,} records in Stage 15 from this run!")
            print("\n   " + "="*60)
            print("   ROLLBACK BLOCKED - DOWNSTREAM DATA EXISTS")
            print("   " + "="*60)
            print(f"\n   What this means:")
            print(f"     Stage 15 has already processed {dep_result.cnt:,} records from run '{validated_run_id}'.")
            print(f"     If we delete Stage 14 data now, those Stage 15 records would become")
            print(f"     'orphaned' - their source data would be gone, corrupting the pipeline.")
            print(f"\n   What to do:")
            print(f"     1. First run the Stage 15 rollback for this run_id:")
            print(f"        python3 rollback_stage_15.py --run-id {validated_run_id}")
            print(f"     2. Then come back and run this Stage 14 rollback again.")
            print(f"\n   Why this matters:")
            print(f"     Data pipelines must be rolled back in reverse order (Stage 15, then Stage 14)")
            ...
            print("   ✅ No downstream data found in Stage 15. It is safe to proceed.")

    except Exception as e:
        # Check if the error is because the table doesn't exist
        if "Not found" in str(e) or "404" in str(e):
            print("   ✅ Stage 15 table does not exist yet. It is safe to proceed.")
        else:
            ...
            print("   " + "="*60)
            print("\n   What this means:")
            print("     The system cannot verify if Stage 15 has data from this run.")
    ...
    # Confirm deletion
    if not confirm:
        print(f"\n⚠️  WARNING: This will delete {record_count:,} records from Stage 14")
    ...
    try:
        ...
        print(f"\n✅ ROLLBACK COMPLETE")
        print("="*70)
        print(f"Stage 14 data for run_id '{validated_run_id}' has been deleted.")
        print("\nWhat this means: The data from this run has been removed.")
        print("What to do: You can now re-run Stage 14 if needed.")
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Rollback Stage 14 data for a specific run_id"
    )
    parser.add_argument(
        "--run-id",
        type=str,
    ...
    if not args.run_id:
        ...
        print("   What to do:")
        print("      1. First, run: python3 rollback_stage_14.py --list-runs")
        print("      2. Find the run ID you want to rollback")
        print("      3. Run: python3 rollback_stage_14.py --run-id YOUR_RUN_ID")
        sys.exit(1)

    success = rollback_stage_14(args.run_id, args.confirm)
    sys.exit(0 if success else 1)
