#!/usr/bin/env python3
"""
Rollback Script for Stage 3

This script allows non-coders to safely rollback Stage 3 data.

Usage:
    python3 rollback_stage_3.py --run-id RUN_ID [--confirm]

What it does:
  - Deletes all data from stage_3 table for the specified run_id
...
def rollback_stage_3(run_id: str, confirm: bool = False) -> bool:
    """Rollback Stage 3 data for a specific run_id.
    ...
    """
    print("="*70)
    print("ROLLBACK STAGE 3")
    print("="*70)
    print("\nThis script will delete Stage 3 data for a specific run.")
    ...
    # Confirm deletion
    if not confirm:
        print(f"\n⚠️  WARNING: This will delete {record_count:,} records from Stage 3")
    ...
    try:
        ...
        print(f"\n✅ ROLLBACK COMPLETE")
        print("="*70)
        print(f"Stage 3 data for run_id '{validated_run_id}' has been deleted.")
        print("\nWhat this means: The data from this run has been removed.")
        print("What to do: You can now re-run Stage 3 if needed.")
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Rollback Stage 3 data for a specific run_id"
    )
    parser.add_argument(
        "--run-id",
        type=str,
    ...
    success = rollback_stage_3(args.run_id, args.confirm)
