...
========================
This script implements THE_PATTERN: HOLD₁ → AGENT → HOLD₂
- HOLD₁: Receives text from Stage 3 (BigQuery → JSONL staging)
- AGENT: LLM correction agent (transforms text with fidelity preservation)
- HOLD₂: Delivers corrected text to Stage 4/5 (JSONL staging → BigQuery)
...
This implementation uses PrimitivePattern for HOLD₁ → AGENT → HOLD₂ flow.

WHAT STAGE 4 DOES:
------------------
✓ Exports Stage 3 data to HOLD₁ (JSONL/DuckDB staging)
✓ Processes L5 messages through LLM AGENT for text correction
✓ Validates text readiness for NLP processing
✓ Writes to HOLD₂ (JSONL/DuckDB staging)
✓ Loads HOLD₂ to BigQuery (claude_code_stage_4 and claude_code_stage_5)

RATIONALE:
----------
- Uses THE_PATTERN for architectural consistency
- HOLD₁ preserves Stage 3 data with fidelity
- AGENT transforms text (LLM correction)
- HOLD₂ preserves corrected text with fidelity
...
script_id = "pipelines.claude_code.scripts._deprecated.stage_4.claude_code_stage_4.py"
...
# 🎓 LEARNING: Primitive Pattern Integration
# Use THE_PATTERN: HOLD₁ → AGENT → HOLD₂
...
from claude_code_stage_4_agent import llm_correction_agent
...
TABLE_STAGE_4 = "claude_code_stage_4"
...
STAGING_ROOT = PROJECT_ROOT / "data" / "staging" / "claude_code_stage_4"
...
def export_stage3_to_hold1(
    ...
    """Export Stage 3 data to HOLD₁ (JSONL).

    🎓 LEARNING: HOLD₁ Export Pattern
    ...
    Args:
        ...
        restart: If True, overwrite existing HOLD₁ files

    Returns:
        Number of records exported
    """
    logger.info(
        "Exporting Stage 3 to HOLD₁",
        extra={
            "run_id": run_id,
            "correlation_id": correlation_ids.get("correlation_id"),
            "component": "claude_code_stage_4",
    ...
    # Skip if HOLD₁ exists and not restarting
    if not restart and HOLD1_JSONL.exists():
        logger.info(
            f"HOLD₁ already exists: {HOLD1_JSONL}",
    ...
    logger.info(
        f"Exported {records_exported} records to HOLD₁",
        extra={
            "run_id": run_id,
...
def load_hold2_to_bigquery(
    ...
    """Load HOLD₂ data to BigQuery (Stage 4 and Stage 5 tables).

    🎓 LEARNING: HOLD₂ to BigQuery Pattern
    After PrimitivePattern completes, load corrected text from HOLD₂ to BigQuery.
    ...
    if not HOLD2_JSONL.exists():
        logger.warning(
            "HOLD₂ JSONL not found, nothing to load",
            extra={"run_id": run_id, "hold2_path": str(HOLD2_JSONL)},
        )
        return {"stage4_count": 0, "stage5_count": 0}

    # Read HOLD₂ JSONL and separate Stage 4 and Stage 5 records
    stage4_records = []
    stage5_records = []
    ...
    with open(HOLD2_JSONL, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                logger.warning(
                    f"Error parsing HOLD₂ record: {e}",
                    extra={"run_id": run_id, "error": str(e), "line": line[:100]},
                )

    logger.info(
        f"Read {len(stage4_records)} Stage 4 records and {len(stage5_records)} Stage 5 records from HOLD₂",
        extra={
            "run_id": run_id,
...
def main():
    ...
    parser.add_argument(
        "--restart",
        action="store_true",
        help="Restart from beginning (overwrite existing HOLD₁)",
    )
    parser.add_argument(
        "--test",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-pattern",
        action="store_true",
        help="Skip PrimitivePattern execution (only load HOLD₂ to BigQuery)",
    )
        ...
        try:
            ...
            if not args.skip_pattern:
                ...
                config = PrimitivePatternConfig(
                    ...
                    batch_size=args.batch_size,
                    pattern_name="claude_code_stage_4",
                )
