#!/usr/bin/env python3
"""
Verification Script for Stage 10

This script allows non-coders to verify that Stage 10 is working correctly.

Usage:
    python3 verify_stage_10.py [--run-id RUN_ID]

What it checks:
  - Table exists
  - Has records
  - L2 Word entities created correctly (atomic level)
"""
...
from shared.constants import get_full_table_id, TABLE_STAGE_10, TABLE_STAGE_8
from shared_validation import validate_table_id, validate_run_id

def verify_stage_10(run_id: str = None) -> bool:
    """Verify Stage 10 is working correctly.

    Returns:
        True if all checks pass, False otherwise
    """
    print("="*70)
    print(f"VERIFYING STAGE 10")
    print("="*70)
    print("\nThis script checks if Stage 10 worked correctly.")
    ...
    try:
        ...
        print(f"   Problem: {e}")
        print("\n   What this means: The system cannot check if Stage 10 worked.")
    ...
    try:
        ...
        if row_count == 0:
            if validated_run_id:
                ...
                print("   What to do:")
                print("      1. Check if Stage 10 ran successfully for this run_id")
                print("      2. Verify the run_id is correct")
                print("      3. Check Stage 10 logs for errors")
            else:
                print("   ⚠️  WARNING: Table exists but has no data")
                print("   What this means: Stage 10 ran but didn't create any records.")
        ...
        print(f"   Problem: {e}")
        print("   What this means: Stage 10 hasn't run yet or failed to create the table.")
        print("   What to do: Run Stage 10 first, then check for errors.")
        return False


    # Check 2: L2 Word entities have correct level
    print("\n2. Checking that all records are Level 2 (Word) entities...")
    try:
        ...
        SELECT
            ...
            COUNTIF(level != 2) as wrong_level_count
        ...
        if result.wrong_level_count > 0:
            ...
            print("   What this means: Some records are not Level 2 (Word) as they should be.")
            print("   What to do: This is a data corruption issue - contact support.")
            all_checks_passed = False
        else:
            print(f"   ✅ All {result.total:,} records are Level 2 (Word) entities")
    except Exception as e:
        print(f"   ❌ Error checking levels: {e}")
        all_checks_passed = False

    # Check 3: Parent links are valid (point to L4 sentences)
    print("\n3. Checking that word parent links point to valid sentences...")
    try:
        ...
        SELECT
            ...
            COUNTIF(parent_id NOT IN (SELECT entity_id FROM `{validated_stage_8}`)) as orphaned_words
        FROM `{validated_table}`
        {where_clause}
        """
        result = list(client.query(query, job_config=job_config).result())[0]
        ...
        if result.null_parents > 0:
            print(f"   ❌ Found {result.null_parents} words with no parent")
            print("   What this means: Some words don't know which sentence they belong to.")
            print("   What to do: This is a data quality issue - run Stage 10 again.")
            all_checks_passed = False
        elif result.orphaned_words > 0:
            print(f"   ❌ Found {result.orphaned_words} words pointing to non-existent sentences")
            print("   What this means: Some words reference sentences that don't exist.")
            print("   What to do: Run Stage 8 first to create sentences, then run Stage 10.")
            all_checks_passed = False
        else:
            print(f"   ✅ All {result.total_words:,} words have valid parent links")
    except Exception as e:
    ...
    if all_checks_passed:
        print("✅ ALL CHECKS PASSED - Stage 10 is working correctly!")
        print("="*70)
        return True
    else:
        ...
        print("  3. Run Stage 10 again if needed")
...
def main():
    ...
    success = verify_stage_10(args.run_id)
