...
class TestIndustryStandardSyncService:
    ...
    def test_init_all_enabled(
        ...
        with (
            patch(
                "truth_forge.services.sync.industry_standard_sync.CDCSyncService"
            ) as mock_cdc_class,
            patch(
                "truth_forge.services.sync.industry_standard_sync.EventDrivenSyncService"
            ) as mock_event_class,
            patch(
                "truth_forge.services.sync.industry_standard_sync.AutoSyncService"
            ) as mock_polling_class,
        ):
            mock_cdc = Mock()
    ...
    def test_sync_contact_now_from_bigquery(
        ...
        with (
            ...
            patch(
                "truth_forge.services.sync.industry_standard_sync.TwentyCRMService"
    ...
    def test_sync_contact_now_from_crm(
        ...
        with (
            ...
            patch("truth_forge.services.sync.crm_twenty_sync.CRMTwentySyncService"),
        ):
            mock_cdc = Mock()
            mock_cdc_class.return_value = mock_cdc
