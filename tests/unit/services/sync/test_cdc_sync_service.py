...
class TestCDCSyncService:
    ...
    def test_init(
        ...
        with (
            patch(
                "truth_forge.services.sync.cdc_sync_service.bigquery.Client"
            ) as mock_bq_client_class,
            patch(
                "truth_forge.services.sync.cdc_sync_service.TwentyCRMService"
            ) as mock_service_class,
            patch(
                "truth_forge.services.sync.cdc_sync_service.ErrorReporter"
    ...
    def test_capture_change(
        ...
        with (
            ...
            service = CDCSyncService()
            with (
                patch.object(service, "_is_event_processed", return_value=False),
    ...
    def test_process_change_event_from_crm(
        ...
        with (
            ...
            patch(
                "truth_forge.services.sync.cdc_sync_service.CRMTwentySyncService"
    ...
    def test_process_change_event_from_supabase(
        ...
        with (
            ...
            patch(
                "truth_forge.services.sync.cdc_sync_service.SupabaseSyncService"
            ) as mock_supabase_sync_class,
        ):
            mock_bq_client = Mock()
            query_job = Mock()
            query_job.result.return_value = []
            ...
            with (
                ...
                mock_supabase_sync.sync_from_supabase_to_bigquery.assert_called_once_with(
                    "uuid-123"
                )
